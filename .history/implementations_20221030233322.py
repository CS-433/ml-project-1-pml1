...
def compute_gradient_reg_logit(y,tx,lambda_,w):
    return tx.T@(sigmoid(tx@w)-y)/len(y) + lambda_*w
...
def reg_logit_loss(y,tx,lambda_,w):
    X_tx = tx@w 
    return ((X_tx<=100)*np.log(1+np.exp((X_tx<=100)*X_tx)) + ((X_tx>100) - (1+y)/2)*X_tx).sum()/len(y) + lambda_*np.linalg.norm(w)
            ((X_tx<=100)*np.log(1+np.exp(X_tx)) + ((X_tx>100) - y)*X_tx).sum()/len(y)
